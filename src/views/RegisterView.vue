    ...
    <div class="flex flex-col items-stretch justify-start px-6 py-4">
        <div class="flex items-center justify-between w-full py-4">
            <div class="flex items-center justify-start w-full">
                <div class="bg-primary rounded-full w-6 h-6 flex items-center justify-center text-white mr-1">1</div>
                <span class="font-semibold text-14 text-primary">Data diri</span>
            </div>
            <hr class="rounded-sm w-full h-1" :class="getStateBg()">
            <div class="flex items-center justify-end w-full">
                <div :class="getStateBg()" class="rounded-full w-6 h-6 flex items-center justify-center text-white mr-1">2
                </div>
        ...
        <div class="mb-5">
            ...
            <p class="font-medium text-12 mb-2 text-grey w-2/3">Masukkan data pribadi untuk dapat mengakses Menu Utama</p>
        </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div class="col-span-2">
                            <select id="gender" class="p-4 rounded-lg font-medium text-12 w-full border border-grey mt-2">
...
export default {
    ...
    methods: {
        nextState() {
            this.state == 0 ? this.state = 1 : this.$router.push({name: ''})
        },
        getStateBg() {
            return this.state == 0 ? 'bg-primary-grey' : 'bg-primary'
        },
        getButtonState() {
            return this.state == 0 ? 'Lanjutkan' : 'Daftar'
        }
    }
}
</script>
